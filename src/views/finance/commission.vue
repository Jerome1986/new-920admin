...
import { useCommissionStore } from '@/stores/modules/commission'
import { commissionRuleUpdateApi, settingTotalRateApi } from '@/api/commissionRule'

// 定义store
const comissionStore = useCommissionStore()
...
const handleSave = async () => {
  if (form.value.id) {
    const { id, createdAt, updatedAt, ...items } = form.value
    await commissionRuleUpdateApi(form.value.id, items)
...
const saveTotalRate = async () => {
  ...
  if (form.value.id) {
    await settingTotalRateApi(form.value.id, form.value.totalRate)
    ElMessage.success('更新成功')
  }
}

onMounted(async () => {
  await comissionStore.commissionGet()
  form.value = { ...comissionStore.commission }
})
</script>
        ...
        <el-row :gutter="16">
          <el-col :span="8">
            <el-form-item label="一级佣金比例">
              <el-input v-model="form.level1Rate" :step="0.01" :min="0" :max="1" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="二级佣金比例">
              <el-input v-model="form.level2Rate" :step="0.01" :min="0" :max="1" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="平台抽成比例">
              <el-input v-model="form.platformRate" :step="0.01" :min="0" :max="1" />
            </el-form-item>
          </el-col>
        </el-row>
            ...
            <el-form-item label="总比例">
              <div class="total-rate-row">
                ...
                <div class="total-actions">
                  <el-button v-if="!isTotalRate" class="brand-btn" type="primary" @click="setTotalRate">设置
                  </el-button>
                  <el-button v-else class="brand-btn" type="primary" @click="saveTotalRate">保存
                  </el-button>
                </div>
              </div>
            </el-form-item>
