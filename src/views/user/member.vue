...
const handleSizeChange = (size: number) => {
  ...
  if (searchValue.value) {
    ...
    userListGet(params.value.pageNum, params.value.pageSize, 'VIP')
  }
}
...
const searchMaterialGet = async () => {
  loading.value = true
  params.value.pageNum = 1
  const searchRes = await userSearchApi(searchValue.value, params.value.pageNum, params.value.pageSize, 'VIP')
...
onMounted(() => userListGet(params.value.pageNum, params.value.pageSize, 'VIP'))
</script>
    ...
    <div class="user-page">
      ...
      <div class="jel-page-table-wrap">
        <el-table :data="userList" class="jel-data-table jel-table-fill" height="100%" style="width: 100%"
          ...
          <el-table-column label="微信昵称" align="center" prop="nickname" width="100" :show-overflow-tooltip="true">
            <template #default="{ row }">
          ...
          <el-table-column label="开始时间" align="center" prop="vipStartTime" width="160">
            <template #default="{ row }">
              {{ formatTimestamp(row.vipStartTime, 2) }}
            </template>
          </el-table-column>
          <el-table-column label="到期时间" align="center" prop="vipStartTime" width="160">
            <template #default="{ row }">
              {{ formatTimestamp(row.vipEndTime, 2) }}
            </template>
          </el-table-column>
